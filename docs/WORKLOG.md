...
### 二、两个真缺陷（一个用户报、一个我实测抓到）

**① 输入岛臃肿 = 宽度溢出（用户报）**：5 个 48dp 图标 + 行内模型 chip 超出岛宽，chip 被挤成
两行并压到图标行上。回上游实测发现：上游这一行是 **4 个 32dp 小圆钮**、图标 16dp，且
**模型选择器根本不在这一行**（在快捷面板）。据此：功能行只留有真实去向的 2 个入口、
取消人工间距、chip 限宽 128dp + 单行省略、内边距收紧（岛体 **126→114dp**）、热区仍 48dp；
并把 chip 从实心珊瑚胶囊改为中性底/次级文字（层级：最强对比应给输入框）。
