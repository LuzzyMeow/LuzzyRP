...
- **页面结构**：`management-view` 惯例（settings-page-header + max-w-2xl 卡列）——
  ...
  **〔2026-09-11 历史存档：该「扁平单页制」属助手原生页（已移除），不再作为约定〕**
...
- **侧栏折叠组（在线 / 高级，v1.5.0 性能对齐）**：上游 `0.32s cubic-bezier(.22,1,.36,1)`
...
> **⚠️ 历史存档（2026-09-11）**：本模块已按用户指示**彻底移除**（代码 / 资产 / 测试 /
> 构建接入全删，见 `CHANGELOG.md` v1.5.0「移除」段）。**本章不再是设计契约**，
> 仅作该页设计演进的历史记录；重启同类功能须重走硬性规定 9 设计门。
...
> **〔2026-09-11 历史存档〕** 下表四行「助手入口｜…」属已移除的助手原生覆盖层编排，
> 相关实现（`MainActivity.animateAssistantOverlay` / `.lsp-assistant-handoff` /
> `ext/luzzy-assistant.js`）**均已删除**，仅作历史记录；**Web 页之间的交接编排（本表上方各行）仍然有效**。

| 助手入口 | 同编排的助手版：WebView 侧侧栏左收 + 原生覆盖层 `alpha 0→1`（同一令牌）；**覆盖层淡入期间保留 WebView 绘制**（`MainActivity` 过渡结束才停绘），被淡化的才是真的「旧页」 |
| 助手入口｜侧栏收起方式 | **点上游自己的汉堡**（`toggleMobileMenu`），由上游状态机摘 `mobile-sidebar-open`——**禁止直接摘类**：摘类不改上游模块状态 `isMobileSidebarOpen`，会让 DOM 与状态脱节、下次点汉堡失效（会话 57 真机实证）。`<html>` 上的 `lsp-assistant-handoff` 只是信号类，让页面交接控制器让位，**不承载任何 transform** |
| 助手入口｜页内禁用二次动画 | 覆盖层进入淡化的 200ms 内（Compose 侧 `entering = true`）**页内不做任何淡入淡出/位移**——「页内容交叉淡化」由覆盖层 alpha 与 WebView 页承担。页内再叠一套会让新页在覆盖层还半透明时就换完，观感读作**硬切**（会话 57 真机实证） |
| 助手入口｜返回侧栏 | 呼出侧栏 = 一次页面交接：覆盖层淡出用 **200ms**（不是 140ms 退出令牌）与侧栏滑入同帧起跑、同时结束；前端调用**按返回值退避重试**而非盲等固定时长。系统返回键 / onExit 走普通退出 140ms |
