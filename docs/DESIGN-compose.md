...
- **用户可调字号（D1，2026-09-14）**：`fontScale` 相对缩放（1f = 默认；换算基准 = 上游正文默认 16px，
  旧版 12–20px → 0.75–1.25）。两条字号 token 体系一起跟随：
  ① `MaterialTheme.typography`（**全 15 个样式**，含未覆盖的 M3 默认——只缩品牌 6 个会半缩放破版）；
  ② `LocalMarkdownTokens`（字号/行高缩放，dp 间距与折叠行数阈值不缩放——上游只缩字体）。
  provide 点 = `LuzzyTheme(fontScale=…)`，宿主（ComposeActivity）持有状态，设置页滑杆实时预览、松手落盘。
  **有意天花板**：散落在各页面的硬编码 `fontSize = N.sp` 不随用户字号缩放（全量收敛是后续独立工作，
  本批按「两条 token 体系」最小改动）；pro-rules 规则 34 的完全合规以此为准。
...
- **系统字号 vs 应用内字号（D1）**：`LocalDensity.fontScale`（系统缩放）与 `fontScale`
  （应用内设置）是**两个独立量**，`HtmlCard.textZoom` 跟随前者、正文跟随后者，不互相替代。
...
| 3 | ~~**预设 / 世界书编辑**~~ | **已完成（2026-09-13，会话 72 W0-W7）**：纸页清单 + 抽页编辑（用户豁免三方向，落档 `boards-v6/`）→ 见 §23。 |
| 4 | ~~**用户可调字号**~~ | **已完成（2026-09-14，D1）**：`fontScale` 落 `SettingsStore`、legacy `fontSize` 按 16px 基准换算补空缺；两条字号 token 体系（`MaterialTheme.typography` 全 15 样式 + `LocalMarkdownTokens`）随缩放，设置页滑杆（12–20px 语义）。设计自由度登记在 §3/§9。 |
| 5 | ~~**迁移报告页**~~ | **已完成（2026-09-14，D3）**：设置页「迁移报告」行 → Dialog（12 项计数 + 时间 + 「未迁移」显式态）；数据源 `kv[legacy.migrationCounts]`，取数 `PageDataSource.migrationReport()`，落档 §29。 |
...
1. **显示期正则**（上游 `applyDisplayRegex` → `processRegex(text, {isDisplay:true, role})`）：
   ...
   没有消费方）。用户 2026-09-13 报的「引号内文字高亮没了」即此项。
   → **2026-09-14 已实现，见 §27。**
2. **UI 模板变量块**（上游 `findUiTemplateUpdateBlock` / `parseUiTemplateUpdates` /
   ...
   同时把变量块从正文里**剥掉**。Compose 版未实现。**待做（静态可做一半，见 §27.6）**。
   协议是标签包裹的 JSON：`<ui_template_updates>{…}</ui_template_updates>`
   （`built-in-content.js:170-188` 的提示词原文）。**注意**：真机上看到的 `!set_color` 一行
   **不是**这套机制——全仓库 JS 里没有任何 `!命令` 解析（2026-09-14 真机核查），
   那是模型自己发明的控制行，上游同样会把它当普通文字显示。别再把它当成本项目的缺口。

---

## 27 · 显示期正则 + 行内 HTML（P5-B 补漏，2026-09-14 · 用户报「引号高亮没了」/「这一段是不是正则」）

### 27.1 问题定位（两次用户反馈，其实是同一族的两半）

| 用户看到 | 归属 | 结论 |
|---|---|---|
| `<div style="background:#0d1416">…</div>` 铺成纯文本 | **块级 HTML** | §26 已修（卡片通道） |
| 引号里的对白没有颜色 | **行内 HTML** | 本节：模型/脚本写在正文里的 `<span style=…>` 从来没被渲染过 |
| 正文第一行 `!set_color` | 模型自造控制行 | **不是**我们的缺口：上游也没有任何消费方（§26.6②） |

也就是说：上游那条「正则 → marked → HTML → DOM」的链，我们此前只补了**块级**那一段
（HTML 直通），**行内**那一段（浏览器渲染 `<span>`）和**链首**那一段（用户正则脚本）都缺。
本节把这两段补齐，消息渲染链自此与上游同形。

### 27.2 实现（三处新增 + 两处收窄 / 接线）

| 文件 | 职责 |
|---|---|
| `chat/RegexScripts.kt`（新，纯函数） | 上游 `processRegex`（`app.js:4014-4091`）的移植：脚本解析、逐条判据、受保护区、JS 替换语义 |
| `ui/markdown/InlineHtml.kt`（新，纯函数） | 行内标签 + CSS 子集 → [MdSpan.Html]（行内渲染的数据形态） |
| `ui/markdown/MarkdownText.kt` | 新增 `MdSpan.Html` 分支：三态映射成 `SpanStyle`；字号沿嵌套**相乘**（等价 CSS `em`） |
| `ui/markdown/HtmlBlocks.kt` | **收窄**：行内标签（span/b/i/u/strong/em/code/font/small…）从 `BLOCK_TAGS` 移出——否则每个高亮都会变成一个 WebView 卡片 |
| `chat/PromptInputSource.kt` | `regexScripts(characterUuid)`：全局 + 当前角色，口径照 `combineRegexScriptsForCharacter`（`app.js:3143`） |
| `ui/markdown/MessageBody.kt` | 链首接线：正则 →（分段）→ Markdown/行内 → 块级卡片；两条路径都只跑一次脚本 |
| `ui/pages/chat/*` | `AiMessagePanel` / `UserBubble` 增加 `scripts` + `userName`；`ChatPage` 随角色载入并记一行日志 |

**逐条对齐上游的语义**（每条都有出处，改之前先读那几行）：
`{{user}}` 占位符（`app.js:627`）／`system` 不套脚本（`:4019`）／「NAI画图正则」恒排最后（`:4020`）／
`enabled`（`:4028`）／`placement`（`:4032`）／显示跳 `promptOnly`、提示词跳「仅用户可见」（`:4037`）／
`minDepth`·`maxDepth`（`:4042`）／`/pattern/flags` 与内联 `(?s)(?i)(?m)`（`:4057`、`core-utils.js:281`）／
匹配式不含 `<>`·围栏时**只改受保护区之外**（`:4072-4084`）／编译失败只跳过该条（`:4086`）／
受保护区（HTML 块·注释·围栏·行内代码·**任意标签**·思考块，`core-utils.js:293-309`）。

**JS 替换串语义**是手写的（`expandReplacement`）：`$&` / `` $` `` / `$'` / `$$` / `$1..$99` / `$<name>`。
Kotlin 的 `Regex.replace` 只认 `$n` 与 `\`，照搬会把 `<span>$&</span>` 原样吐回正文——
而「整段包裹高亮」正是用户脚本最常见的写法。

### 27.3 行内 HTML 的语义取舍（与浏览器/上游的差异，逐条明示）

| 情形 | 本节行为 | 浏览器 |
|---|---|---|
| 认得的标签/样式 | 映射为样式片段 | 渲染 |
| 不认得的标签 | 丢标签留内容 | 内容可见 |
| `script`/`style`/`iframe`/`object`/`embed`/`template`/`noscript`/`svg`/`math` | **连内容一起丢** | 执行 / 影响整页样式 |
| `<br>` `<hr>` | 换行 | 换行 / 画分隔线 |
| `<img>`（行内） | 丢；有 `alt` 时显示 alt 文字 | 加载图片 |
| `class` / `<style>` 里的 CSS | **不生效** | 生效 |
| `mark` 等浏览器默认样式 | 不套（只认显式声明） | 有默认样式 |

样式子集：`color` `background(-color)` `font-weight` `font-style` `text-decoration` `font-size`
（`%`·`em`·`px`·关键字）+ 语义标签固有样式（b/i/u/s/code/small/big/sub/sup/font）。
颜色解析支持 `#rgb` `#rgba` `#rrggbb` `#rrggbbaa` `rgb()` `rgba()` 与约 40 个常用具名色；
**解析不出就不改样式**（不塞猜测值）。

**安全**：行内层不执行任何东西——它只产出结构化片段，交给 Compose 文本渲染。
`<a href>` 只放行 `http/https`（其余退化为普通样式文本），与 §26.3 的卡片边界同一口径。

### 27.4 设计门记录（硬性规定 9）

**豁免②（在已选定方向上的迭代）**：本节的视觉结果 100% 来自**用户脚本/模型自己写的 CSS**，
我们没有发明任何新视觉方向；与 §26 同属「让正文与上游同形」这一条已评审过的方向，
故不再走三方向门，但把**本次唯一的三处设计自由度**如实登记：

1. **字号倍率夹在 0.6×–2.0×**（模型写 `font-size:800%` 时上游是整页撑开，气泡宽 336dp 下是灾难）；
2. **不套浏览器默认样式**（`mark` 不给黄色底）：颜色/字号来源必须可回答「是谁给的」；
3. **`class` 不生效**：我们没有应用级样式表可查，凭空支持等于自己编一套映射。

**残余风险（登记不修）**：① 模型给的配色可能与主题冲突（与 §26.7 的卡片配色同源问题）；
② 行内样式**不参与**主题切换——这是「谁写的谁负责」的必然结果；
③ 一条正文里若含**大量**行内片段，`AnnotatedString` 会变长（当前无实测样本）。

### 27.5 状态：静态完成，**目视验收未完成**（设备在验收中途被收回）

已完成并可判定：
- JVM 门禁 **647 条 / 0 失败**（新增 `RegexScriptsTest` 34 条 + `InlineHtmlTest` 24 条 + `HtmlBlocksTest` 1 条）；
- 真机（`df97f3c4`）装包后：**块级卡片正常**（与 §26.8 同一份证据链），消息渲染无异常；
- 真机日志读到用户真实数据：**显示期正则共 1 条 —— `NAI画图正则`**（系统脚本，非高亮类）。

**未完成（下一轮/真机回来时补，清单见 `docs/HANDOFF-*.md`）**：
行内高亮的**目视**证据没拿到——用户会话里现存的 AI 消息都是纯引号（无 `<span>`），
用户自己那条 `NAI画图正则` 只匹配 `image###…###`。要拿到证据需要**发一条含行内标签的消息**
（写用户会话数据 → 需用户在场同意）。

### 27.6 本节**未做**的部分（如实登记；会话 76 已划掉三项，见下）

> **2026-09-14（会话 76，无真机轮）进度**：第 1、2、3、4 项**已完成**（静态可判定的部分），
> 第 5 项按用户拍板**显式跳过**，第 6 项仍维持现状。详见下面逐条。

1. ~~**提示词侧正则**~~ **✅ 已完成（会话 76）**：原记「函数已支持但没接进 `RequestBuilder`」，
   本轮接上了——`RequestBuilder.plan(regexScripts, promptUserName)` →
   `RegexScripts.applyToPromptMessages`（对**整条已装配好的消息序列**套用，
   `depth = 条数-1-下标`，与上游 `app.js:6511-6518` 同）。
   **两条必须知道的事**：
   - 只有勾了「仅提示词」的脚本会改请求（默认什么都不勾 = 仅用户可见 → 提示词侧一律跳过），
     所以绝大多数用户的脚本**一个字节都不改请求**，批 A 的前缀性质不受影响；
   - 勾了「仅提示词」**又有深度区间**的脚本会让前缀在同一位置逐轮漂移——
     这是深度语义的固有后果（上游亦然），`RequestBuilderTest` 把它写成**已知代价用例**，
     真机复核命中率时若见下降先看这里（`HANDOFF-p5-static.md` B3 有排查口径）；
   - `{{user}}` 在提示词侧按**档案原名**替换（不套显示期的「你」兜底），空名保留占位符。
2. ~~**文风过滤**~~ **✅ 已完成（会话 76，用户拍板「做，默认开，照上游」，并在同日扩为「完全照上游」）**：
   `chat/StyleFilter.kt`，接在**三处**（与上游同）：
   - ① **显示期**（`MessageBody`，接在正则与 HTML 之后）；
   - ② **提示词侧**（`RegexScripts.applyToPromptMessages`）——上游把这一调用放在 `processRegex`
     出口（`app.js:4090`），而提示词侧走的正是 `processRegex`（`app.js:6513`），
     所以**发给模型的 assistant 历史也是过滤后的**；
   - ③ **生成收尾落库前**（`ChatPage.resultOf`）——上游在 `app.js:6872-6875` 就地改写
     `assistantMessage.content` 再存盘。
   **三处必须同源同开关**：少做任何一处都会得到「界面上被删了、发给模型的还在」这种
   自相矛盾的状态。三处都走 `StyleFilter.filter`，判据只在那一份实现里。
   **代价（用户已知悉并拍板）**：③ 让**库里存的正文也是过滤后的**，新生成的 AI 正文里
   模型原话**不再留存**（不可逆）。
   **三处必须知道的事**：
   - 它对白保护只认 `“”` / `『』` / `"`（上游原文如此），**`「」` 不在保护集内**——
     直角引号里的黑名单短语**会被删**。这是照抄上游的结果，不是漏做；
   - **整句档会连短语前面的同句内容一起删**（实测：「他向前一步。指尖因为用力而发白，然后开口。」
     → 「他向前一步。」）；
   - 两个**分句档被整句档完全遮蔽**（短语集是子集、且整句档在前）——实测从不触发。
     规则照抄保留，但用例断言的是观测到的真值；
   - 与上游的**有意偏离**：只删、**不高亮**命中片段（高亮是视觉产出，要过设计门 + 真机）。
3. ~~**UI 模板变量块**~~ **✅ 剥除已完成（会话 76）**：`chat/UiTemplateUpdates.kt`
   （`strip` / `find` / `parse`，28 条单测），接进 `MessageBody` 与 `UserBubble`。
   **顺序是「先剥块、再跑正则」**（上游 `app.js:11016` 同）——于是块内 JSON
   既不被正则改写、也不进行内 HTML／卡片分段。
   **面板渲染（`renderUiTemplateHtml`）仍未做**，故 `Parsed` 的结果**目前没有消费方**
   （只有测试在用）。这是有意的半步，别当成「已经能用了」。
4. ~~**思考块（CoT）拆分显示**~~ **✅ 纯逻辑部分已完成（会话 76）**：
   `CotParser` 补齐 `ranges` / `rawCot` / `closingTags` / `isFinished`（完整移植），
   并**收编**了 `RegexScripts.cotRanges` 的重复实现——受保护区自此只有一份真源
   （两套实现意味着「哪段算思考块」有两个答案，而受保护区判错是**静默**的）。
   **渲染成节点的视觉仍未做**（缺陷 #4 的目视一半），留真机。
5. **NAI 生图管线**：~~本节只让它能正确套用~~ **✅ 按用户拍板显式跳过（会话 76）**：
   `RegexScripts.IMAGE_GEN_UNSUPPORTED = true` 拦下整条脚本。理由：替换产物是**生图卡片**
   （`app.js:9647-9650`），而卡片图片靠上游 JS 管线填、我们的卡片**网络被禁**（§26.3）
   → 放行只会得到一张**永远不出图的空卡片**，比保留 `image###…###` 原文更糟（看不出是没实现还是坏了）。
   开关是一行常量，生图管线（批 C C4）做好后翻它即可恢复；「恒排最后」「内置匹配式」两条语义照旧实现。
6. **用户消息的 Markdown**：`UserBubble` 只接了「正则 + 行内样式」（+ 本轮新增的变量块剥除），
   没接 Markdown 渲染（用户气泡按原样显示是既有设计决定）。

### 27.7 会话 76 新增：变量块与文风过滤在链上的**顺序**

上游的正文渲染链在本轮的实现里是四段（顺序有出处，不可颠倒）：

| 段 | 做什么 | 上游出处 |
|---|---|---|
| ① 剥变量块 | 去掉 `<ui_template_updates>…</ui_template_updates>` 及其之后 | `app.js:11016` |
| ② 显示期正则 | 用户脚本改写正文（可产出 HTML） | `app.js:4014` / `runtime-services.js:22` |
| ③ 行内 + 块级 HTML | 行内标签→样式片段；块级→卡片 | 浏览器 / §26 |
| ④ 文风过滤 | 删掉 AI 腔调短语（**只对 assistant**） | `app.js:4090` |

**①② 的顺序是硬要求**：块里是 JSON（`{"hp": 3}`），先跑正则的话
「把数字高亮」这类脚本会把块里的 `3` 也包成 `<span>`，剥除时就再也认不出原文了。
**④ 必须在 ②③ 之后**：它筛的是正则处理完的文字，而且它自己也要走受保护区
（代码/思考块不动）。

### 27.8 会话 76 的**两处静默失效**（都被测试当场抓住，如实登记）

两处都是「不报错、不崩溃、只是行为悄悄不对」的类型——正是本仓库最该记下来的那类：

1. **提示词侧占位符被删空**：`{{user}}` 的「空名保留占位符」这条契约，原先**不是显式实现**，
   而是靠 `applyToPromptMessages` 的提前返回条件**顺带**兜住的。本轮加②（提示词侧文风过滤）
   后，提前返回多了一个分支（「名字为空但过滤开着」会走进 `apply`），占位符当场被删成空白 →
   `RequestBuilderTest.提示词侧的 user 占位符按档案名替换` 立刻红。
   **修法**：把判据搬到源头（`RegexScripts.apply` 里 `if (userName.isBlank()) text`），
   这样调用方再增分支也不会失效。
   **教训**：一条语义不能靠「调用方的提前返回」来实现——那是隐式契约，多一个分支就破。
2. **记忆页读主线而不是当前分支**：会话的本体是 (角色, **分支**) 这一对，而第一版
   `PageDataSource.memory` 用的是 `ScopeId(uuid)`（默认落到主线 `main`）。
   用户的活跃会话**经常不在主线上**（仪器化夹具里就是 `b1`）→ 记忆页会显示 0 条，
   而对话里明明有记忆。**修法**：`memory(uuid, branchId)` + `activeBranch()`，
   仪器化用例刻意用一个**非主线**的活跃分支来跑（用主线就测不到它）。

---

## 28 · 批 C 余项的设计落点（2026-09-14 回填，会话 78 欠账）

按 awesome-design-md 九节结构补记三处批 C 交付的设计决策（实现先于落档，本节补账）：

- **C3 · 候选切换器**（`‹ n/m ›` 持久化）：沿用 §14.5 的操作行与气泡名行右侧既有 chip 形态，
  无新视觉语言；候选集合以 payload 私有键 `luzzyCandidates`/`luzzyCandidateIndex` 持久化
  （不改表——原判据「要改表」不成立），杀进程重开认回。
- **C4 · 附件缩略图条**：输入岛上方横排 56dp 圆角缩略图（`RoundedCornerShape(14.dp)`），
  每格右上角 `error` 色圆形移除钮；缩略图条只在有待发附件时出现（占位策略：内容驱动显隐，
  不留空槽）。
- **C7 · reduced-motion 口径**：共享层 `ui/MotionSettings.kt`（ContentObserver 订阅）+
  纯函数折算，口径 = `ANIMATOR_DURATION_SCALE == 0f` 才停（**0.5 是「快」不是「停」**；
  NaN 按照常播）；应用到 8 处自绘动效，`HtmlCard` 收编同一读取器（与 §26 的 textZoom 并列）。

## 29 · 导入导出与迁移报告（D2/D3，2026-09-14）

- **D2 · 设置页「数据 · 导入与导出」卡**：六行 = 导出（预设/世界书/角色卡）+ 导入同三项；
  行式复用 `SettingRow`（既有一行式，无新组件）；SAF launcher 在宿主，结果用 Toast 一行反馈。
  覆盖语义在界面上写明（「整组覆盖现有预设」「同卡覆盖；外来卡新建」）——**导入 = 这次导入就是
  当前状态**，与初次迁移的「uuid 合并」是两种意图。格式：预设/世界书 = `records` 原样数组
  （零映射）；角色卡 = 单对象或数组、`data` 外壳与顶层平铺都原样保留、**导出注入 uuid**
  （迁移器补的身份不在 payload 里，不注入会让导出→导入重复入库）。
  **运行时 SAF 验证留真机**（无设备），如实登记。
- **D3 · 迁移报告行 + Dialog**：入口在数据卡底部（「迁移报告 · 从旧版搬来了什么」）；
  Dialog 列 12 项计数（`kv[legacy.migrationCounts]` 的键序叙述：角色 → 会话 → 记忆 → …）
  + 尾注「迁移于 <时间> · 迁移对旧数据只读」；**无记录显式呈现「本机没有迁移记录」**
