...
> [Release](https://github.com/LuzzyMeow/LuzzyRP/releases)；遇到问题或有想法，欢迎提
> [Issue](https://github.com/LuzzyMeow/LuzzyRP/issues) 反馈。
...
| v1.3.0 | 上游 1.9.1 同步（传输层重构适配/抗Gemini截断/UI模板协议检查/破限标记 rp_hub_default）+ 性能治理（流式降载 / 高频面退实底 / 合成层瘦身 / 开屏去 blur / 发送键热区根除，patch 032-034）+ 供应商图标（相册选图 1:1 裁剪，patch 035）+ 内置供应商精简仅留 DeepSeek 且可编辑（patch 029，含老用户无损迁移）+ 记忆召回思考节点（patch 031）+ 关于页上游版本号固化（patch 030）+ 真机反馈修复（冲突误报/名称截断/模型来源澄清）+ 工坊页 JS 存量缺陷修复（✅ 已发布，附 APK） | ✓（稳定版） |
