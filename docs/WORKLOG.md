...
## 📍 当前工作节点（会话 75 暂停 · 批 B 代码完成，真机未验收 · 供下次接手直接续）

> **接手第一件事读这一节 + `docs/HANDOFF-p5-batch-c.md`**（AGENTS §3.1）。
> HEAD `ec96fb1c`（**未 push**，本节点与文档在同一批提交里推）。
> **本轮主文档**：`docs/PLAN-v3.0-p5-agent-loop.md`（§3 批 A / §4 批 B / §7 验收 / §11 暂停点与真机记录）。
...
| 项 | 值 |
|---|---|
| 五阶段 | Stage 0-4 完成（含真机迁移）；**P5 批 A 完成并真机验收达标**；**批 B 代码完成（B1-B7 全部落地），真机未验收** |
| HEAD | `ec96fb1c`（代码）+ 本轮 docs 提交 |
| 门禁 | **JVM 单测 561 条 / 0 失败（3 连跑稳定）**；`checkChat`（仪器化）**按用户指示未跑**；仪器化源码编译通过 |
| 真机 | `df97f3c4`（小米 25098PN5AC / Android 16）；**机上仍是 `d400e766`**（批 A + 输入法修复）；批 B 未上机，**装包前须问用户** |
...
1. **真流式输出失效**——**静态 A/B 已排除「批 A 回归」**（证据见会话 75 §2）；推断与「不吸附」
   同机理（follow 掉出后新文字在视口下方）。**待真机 A/B 定性**，之前不得写成「既有缺陷」。
2. **划到底部不自动吸附跟随**——同上，`isAtBottom()` 的既有缺口。
...
6. **供应商配置保存丢字段**（会话 75 §6-1）——**已修**（`initial.copy`）。

### 真机环境注意事项（踩过的坑）

- **模拟器**：用户明确要求**不用**（不要启动）；`checkChat` 因此跑不了，门禁用 `:app:testDebugUnitTest`。
- **真机文本注入不可用**：`adb shell input text` / `input keyevent` 打不进 Compose 输入框；
  能用的是 `input tap` + `uiautomator dump` 取坐标；**需要发消息就请用户发**。
- **不要再动真机输入法设置**。
- 真机数据是**用户的真实数据**：写之前先问。
- 装批 B 包：`./gradlew :app:assembleRelease` →
