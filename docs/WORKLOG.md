...
| 项 | 值 |
|---|---|
| 五阶段计划位置 | **Stage 0/1/2/3 全部完成；Stage 4（P4-C 收口）4 项里完成 3 项** |
| HEAD | `08d2eeb7` ＋ 本次「计划落档」文档提交（会话 72） |
| 门禁 | 单测 **358** + 仪器化 **33** 全绿；`ANDROID_SERIAL=emulator-5554 ./gradlew checkChat`（54s） |
| 最新发布 | 仍是 **v1.4.0**；v2.0.0 / v3.0.0 都**未发版** |
| 设计真源 | `DESIGN-compose.md`（UI，已到 §22）／`DESIGN-migration.md`（数据层，§1-§11） |

### Stage 4（P4-C）剩余两项

1. **预设 / 世界书编辑**（计划 4.2）：**计划已落档** `docs/PLAN-v3.0-presets-worldbook.md`（会话 72），
   **用户已豁免三方向**（Gate 文件 `docs/design/boards-v6/direction-approved-v6.md`）→ **可直接开工，
   从 W0 起**（W0 = 旧键裁决修正 —— 先改数据口径，再动 UI）。
2. **真机覆盖安装实测**（计划 4.3）：需用户授权——会替换其日常应用（小米 A9210 / `PA921BMGL3190210G`）。
   步骤与回滚已写进 `DESIGN-compose §20.4`；⚠️ 该机**未安装 debug 包**，只有 release，注意别用 debug 装。

### 顺着计划再往后（P5 / P6）

- **P5 功能对账**：Markdown 的 LaTeX / Mermaid / HTML 直通 / 图片、表格列宽自适应、代码高亮；
  以及顺延项 —— **真实角色图解码**（现为 monogram 降级）、**用户可调字号**（需设计门）、
  **迁移报告页**（需设计门）、附件/预设/工作区三个入口的真实功能、多候选 `‹ n/m ›` 持久化。
...
2. **下一步已明朗**：做预设/世界书编辑（计划已落档、设计门已豁免三方向）。
   计划里两项**告知**（不同意就说一声）：本批只做管理面、**「生效」归 P5**；**导入/导出也归 P5**（不留假按钮）。
