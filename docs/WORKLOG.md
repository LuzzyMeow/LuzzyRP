...
> ③ 跑 `./gradlew :app:testDebugUnitTest` 确认 **561 条全绿**（接手基线）。
...
| 项 | 值 |
|---|---|
| HEAD | `2db9393a`（代码修复 `2c747503` + 文档，**均已 push**） |
| 门禁 | **JVM 单测 561 条 / 0 失败（3 连跑稳定）**；`checkChat`（仪器化）**按用户指示未跑**（无模拟器）；仪器化源码编译通过 |
| 真机 | `df97f3c4`（小米 25098PN5AC / Android 16）；**机上已是 `2c747503`** 那份 release 包（批 A + 批 B + 跟随修复；同签名覆盖、数据保留） |
| 阶段 | Stage 0-4 完成（含真机迁移）；**批 A 真机验收达标**（公共前缀 1.0000、命中 89%）；**批 B 代码完成 + 真机抽验通过**；**批 C 未开始** |
| 设计真源 | `DESIGN-compose.md`（§24 请求组装与缓存 / §25 Agent Loop 已回填；§15.3 供应商配置已更新） |
...
| 3 | 正则高亮缺失 | ❌ Compose 版从未实现正则显示应用 → 功能缺口（批 C） |
...
4. **两项遗留验证**（不阻塞上面）：① B6 的真机触发（要真断网/真溢出，无法按需造）；
