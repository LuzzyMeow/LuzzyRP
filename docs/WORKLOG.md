...
> **接手第一件事读这一节 + `docs/HANDOFF-p5-batch-c.md`**（AGENTS §3.1）。
> HEAD `2c747503`（**已 push**）。本轮主文档：`docs/PLAN-v3.0-p5-agent-loop.md`
> （§3 批 A / §4 批 B / §7 验收 / §11 暂停点与真机记录）；真机细节见本文件「会话 75 §10」。

### 状态一览

| 项 | 值 |
|---|---|
| 五阶段 | Stage 0-4 完成（含真机迁移）；**P5 批 A 真机验收达标**；**批 B 代码完成 + 真机抽验通过**（B5 压缩已在真机跑出；B6 重试真机不可按需触发） |
| HEAD | `2c747503` |
| 门禁 | **JVM 单测 561 条 / 0 失败（3 连跑稳定）**；`checkChat`（仪器化）**按用户指示未跑**；仪器化源码编译通过 |
| 真机 | `df97f3c4`（小米 25098PN5AC / Android 16）；**机上已是 `2c747503`** 那份 release 包（批 A+B+跟随修复，同签名覆盖、数据保留） |
| 设计真源 | `DESIGN-compose.md`（§24/§25 已回填，§15.3 已更新） |
| 最新发布 | 仍是 v1.4.0；v3.0.0 未发版 |
| 用户两条反馈 | **已定性并修复**：不是流式链路的问题，是「跟随链」三处缺陷（本文件 §10-①） |
...
| B3 工具配对 + 合成结果 + 工具抛异常兜底 | ✅ |
| B3b 工具轨迹落库 | ✅ `chat/ToolTrail.kt` + payload 键 `luzzyToolTrail` |
| B4 中断保留（`interrupted` 落库/认回） | ✅（**UI 显示未做**，设计流程） |
| **B5 压缩** | ✅ 本轮完成（`Compaction` + 水位线落库 + `contextWindow` 设置项） |
| **B6 重试** | ✅ 本轮完成（`RetryPolicy` + 两类各一次） |
| B7 纯追加不变量守卫 | ✅ |

### 未修的真实缺陷（用户实测反馈，已分诊）

1. **真流式输出失效**——**已定性并修复**（本文件 §10-①）：三处「跟随链」缺陷（贴底不到位 /
   开页贴底是空操作 / 跟随判据抖动即永久失效）。**不是流式链路本身的问题**（真机采样可见
   可见字数逐帧增长）。修复已上机复验。
2. **划到底部不自动吸附跟随**——同上，同一批三处缺陷的另一面。
3. **正则高亮缺失**——Compose 版从未实现正则显示应用 → 功能缺口（批 C）。
4. **首轮正文出现 CoT、输出完毕后消失**——流式期与收尾后显示不一致 + 未识别成节点（批 C）。
5. **追加消息覆盖老消息**（会话 75 §6-2）——**已修**（`MAX(sortIndex)+1`）。
6. **供应商配置保存丢字段**（会话 75 §6-1）——**已修**（`initial.copy`，真机保存后其余字段未变）。
7. **配置对话框在「已配置」后不可达**（会话 75 §10-③）——**已修**（模型面板底部补入口，真机可用）。

### 真机环境注意事项（踩过的坑）

- **模拟器**：用户明确要求**不用**（不要启动）；`checkChat` 因此跑不了，门禁用 `:app:testDebugUnitTest`。
- **文本注入**：聊天输入岛确实打不进；但**对话框里的文本框可以**——`input keyevent` 逐位数字最可靠
  （数字键码 0=7…9=16），`input text` 会经 IME 合成路径插入垃圾字符。
- **弹窗按钮会被软键盘盖住**：uiautomator 给的按钮 bounds 不含键盘 → 先发 Enter（66）收键盘，
  再按收起后的坐标点按钮（详见 §10-④；本轮曾因此误改「模型」字段，未保存已恢复）。
- **不要再动真机输入法设置**。
- 真机数据是**用户的真实数据**：写之前先问；本轮登记见 §10-⑤（含一条压缩水位线的副作用）。
