...
| HEAD | `8a2433aa`（已 push；上一批实质提交是 `2c95edae` 会话总览页 + 性能专项） |
...
### Stage 4（P4-C）剩余两项 —— 都**不在我单方面能推进的范围**内

1. **预设 / 世界书编辑**（计划 4.2，现为只读列表）：新增交互 → **硬性规定 9 要求先出 3 个差异化方向给用户选**，
   我不能直接开写。数据层与页面骨架都已在（`records` 表 + 现有只读面板）。
...
1. **真机覆盖安装**：是否授权我在其手机上 `install -r` v3.0 包并触发迁移（同签名、数据保留、旧库只读）。
2. **下一步做哪个**：预设/世界书编辑的设计门（我出 3 方向）／还是先做 P5 里**不需要设计门**的那些
   （角色图解码、Markdown 补齐）。
