...
1. **提示词侧正则**（上游 `processRegex(..., {isPrompt:true, depth})`，`app.js:6511-6518`）：
   函数已支持 `Mode.Prompt` 与 `depth`，但**没有接进 `RequestBuilder`**。
   未接的理由：它改变发给模型的内容，与批 A 的「纯追加前缀」不变量相邻，
   需要连同 `RequestBuilderTest` 一起改并重跑缓存验收——**不能顺手塞**。
2. **文风过滤**（`filterBlockedStyleText`，`app.js:1470`，`settings.styleFilterEnabled` 默认**开**）：
   上游会在显示前删改 AI 措辞并高亮命中片段。我们没实现 → 我们会比上游**多显示**那些字。
   单独立项（它删正文，与正则不是一件事）。
3. **UI 模板变量块**：`<ui_template_updates>{json}</ui_template_updates>` 的**剥除**（纯函数，静态可做）
   与其**面板渲染**（要真机）。
4. **思考块（CoT）拆分显示**：本节只把思考块纳入**受保护区**（正则不改它），
   「把 thinking 单独渲染成节点」仍未做（对应缺陷 #4）。
5. **NAI 生图管线**：`NAI画图正则` 的替换产物是一张**生图卡片**
   （`<div class="generated-image-card is-generating" data-image-request="…">`，`app.js:9647-9650`）。
   本节让它能正确套用，但卡片的图片是上游 JS 管线填的 → 我们只会得到一个**空卡片**
   （且卡片网络被禁，见 §26.3）。生图/附件管线属批 C（C4），**别把它算进本节**。
6. **用户消息的 Markdown**：`UserBubble` 只接了「正则 + 行内样式」，没接 Markdown 渲染
   （用户气泡按原样显示是既有设计决定）。
