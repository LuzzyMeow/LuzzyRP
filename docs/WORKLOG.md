...
## 📍 当前工作节点（会话 75 暂停 · 批 B 代码完成并在真机抽验 · HTML 直通落地 · 供下次接手直接续）

> **接手三件事**：① 读这一节；② 读 `docs/HANDOFF-p5-batch-c.md`（下一轮的完整提示词）；
> ③ 跑 `./gradlew :app:testDebugUnitTest` 确认 **588 条全绿**（接手基线）。
> 主线文档：`docs/PLAN-v3.0-p5-agent-loop.md`（批 A/B 详案与验收）、`docs/PLAN-v3.0-compose.md`（P6 发版）。
> **真机细节**：本文件「会话 75 §10」（含三处缺陷的设备证据、压缩日志、设备操作经验、写操作登记）。

### 状态一览

| 项 | 值 |
|---|---|
| HEAD | `062796ad`（代码修复 `2c747503` + HTML 直通 + 文档，**均已 push**） |
| 门禁 | **JVM 单测 588 条 / 0 失败**；`checkChat`（仪器化）**按用户指示未跑**（无模拟器）；仪器化源码编译通过 |
| 真机 | `df97f3c4`（小米 25098PN5AC / Android 16）；**机上已是 `062796ad`** 那份 release 包（批 A + 批 B + 跟随修复 + HTML 卡片；同签名覆盖、数据保留） |
| 阶段 | Stage 0-4 完成（含真机迁移）；**批 A 真机验收达标**（公共前缀 1.0000、命中 89%）；**批 B 代码完成 + 真机抽验通过**；**批 C 未开始** |
| 设计真源 | `DESIGN-compose.md`（§24 请求组装与缓存 / §25 Agent Loop / **§26 正文 HTML 直通**；§15.3 供应商配置、§16.4 已同步） |
| 发版 | 最新发布仍是 **v1.4.0**；**v3.0.0 未发版**。⚠️ **launcher 仍指向 WebView 的 `MainActivity`**，Compose UI（`ui.ComposeActivity`）目前只能用 `adb am start` 进 —— 切 launcher 属 **P6**（PLAN-v3.0-compose）|
| 用户两条反馈 | **已定性并修复**：不是流式链路的问题，是「跟随链」三处缺陷（§10-①）；修复已上机复验 |
| 正文 HTML 直通 | ✅ 已实现并真机目测（亮/暗双主题）——见 §11 与 `DESIGN-compose §26`；**同族的另两条上游显示机制仍未做**（显示期正则 / UI 模板变量块，见 §26.6） |
...
| # | 缺陷 | 状态 |
|---|---|---|
| 1 | 真流式「感觉不到逐字」 | ✅ **已定性并修复**：跟随链三处缺陷（§10-①），非流式链路本身 |
| 2 | 划到底不自动吸附跟随 | ✅ 同上（同一批三处缺陷的另一面） |
| 3 | 正则高亮缺失 | ❌ Compose 版从未实现正则显示应用 → 功能缺口（**下一轮第 1 件事**，见 §26.6①） |
| 4 | 首轮正文出现 CoT、输出完毕后消失 | ❌ 流式期与收尾后显示不一致 + 未识别成节点（批 C） |
| 5 | 追加消息覆盖老消息（丢数据） | ✅ 已修（`MAX(sortIndex)+1`） |
| 6 | 供应商配置保存丢字段 | ✅ 已修（`initial.copy`；真机验证保存后其余字段不变） |
| 7 | 配置对话框「已配置」后不可达 | ✅ 已修（模型面板底部补入口，真机可用） |

### 下一步（按序，含理由）

0. **同族的另两条上游显示机制**（`DESIGN-compose §26.6`，用户已就「显示层没还原」提过两次）：
   **① 显示期正则**（上游 `applyDisplayRegex` → `processRegex(text, {isDisplay:true, role})`：
   用户的正则脚本应在**显示前**改写正文，`rp_hub_regex` 记录已迁移进来但没有消费方——
   用户报的「引号内文字高亮没了」即此项）；**② UI 模板变量块**（模型输出 JSON 变量块 →
   渲染模板面板 + 从正文剥掉该块）。
   *理由*：这两条与刚做完的 HTML 直通同属「上游正文渲染链」，用户能直接看见差别，且改动面小。
1. **批 C · C1「五个假数据页接真库」**（PLAN §5-C1）——关于（版本走 `BuildConfig` + CHANGELOG 真源）／
   用量（`records(usage)` + 折线 + **缓存命中率列**，数据源已在 `CacheObserver`）／记忆（`memories` 真统计 + 清空真执行）／
   角色卡（真列表 + 真实头像 + 切换 + 删除）／设置（用户档案真绑定 → 喂 A2 用户信息块；供应商配置接 `TransportStore`）。
   *理由*：它是「界面可信度」的总开关，也是把本轮的 `CacheObserver`/`TransportStore` 真正呈现在用户面前的一步。
2. **批 C 其余项** C2 真实角色图 → C3 多候选持久化 → C4 附件真功能 → C5 表格列宽 → C6 撤工作区 → C7 reduced-motion
   （每项一条断言；**可见性判据优先于数据判据**——「DOM 里有」不等于「用户看得见」）。
3. **批 D**（字号 / 导入导出 / 迁移报告页）→ 然后 **P6 切换与发版**（切 launcher、移除 WebView 路径、
   单 APK + 签名一致 + 真机回归，按 AGENTS §3.4）。
4. **三项遗留验证/风险**（不阻塞上面）：① B6 的真机触发（要真断网/真溢出，无法按需造）；
   ② 「末项远高于视口」的长回复跟随（本轮生成长度不足以把气泡顶出视口）；
   ③ HTML 卡片的滚动帧率（每卡一个 WebView，需「同会话 10 张卡」A/B 才能定性，
   另登记：卡片配色不随主题、WebView 抢占长按选择）。
5. **等用户拍板**：主分支里那条压缩水位线（§10-⑤）是否清掉——清它会连带删掉其后消息，**必须用户确认**。
...
- **弹窗按钮会被软键盘盖住**：uiautomator 给的按钮 bounds 仍是对话框窗口的坐标，键盘绘制在其上 →
  先发 **Enter（66）收键盘**，再按**收起后**的坐标点按钮（本轮因此误改过「模型」字段，未保存、已恢复）。
- **每次点按都要用「当次 dump」的坐标**：对话框在「有键盘/无键盘」两种布局下按钮 y 相差约 300px。
- **不要再动真机输入法设置**；真机数据是**用户的真实数据**：写之前先问；本轮登记见 §10-⑤。
- 装包：`./gradlew :app:assembleRelease` →
  `adb -s df97f3c4 install -r app/build/outputs/apk/release/app-release.apk`（同签名覆盖、数据保留）；
  发布前 `apksigner verify --print-certs` 核对指纹（本轮实测 `ed78235d…` 未变）。
