...
>
> **执行状态（2026-09-10 更新）**：**P0 ✅ 已完成 · P1 ✅ 已完成**（观感项待真机复验）·
> **P2 ⏳ 待用户选方向**（D1/D2/D3 方向板未出）· **P3 ⏳**。落地明细见 §3 末「执行记录」。
...
| **P2** | ⏳ 未动 | 顶栏语言统一需 D1/D2/D3 方向板 → 用户选定 → `direction-approved-assistant.md` | 见该档末节「⏳ 待决」 |
...
| 风险 | 说明 | 对策 |
|------|------|------|
| P2 方向未定 | P1 修完后仍有 A1 断层 | P2 必须等用户选方向再动；P0/P1 不依赖 P2，可先行 |
| 真机缺失 | 本次审查为代码层结论，观感类（C3 间距、B1 色差）需目测 | 实施前请连接设备 `df97f3c4`；否则 P1-7 / P2 的观感项标「待真机复验」 |
| 未提交改动堆积 | 会话 49+50 共 21 文件在途 | 建议 P0/P1 之前**先提交现有 21 文件**（跑一次单测 + 门禁 + 构建），避免修复与在途改动互相污染 |
| 暗色模式 | B1 用户气泡、P1-3 的 coral-300 边需暗色对应值 | 改前先查 `LuzzyColorsDark` 是否已有对应 token，没有则按 DESIGN.md 暗色表补 |
