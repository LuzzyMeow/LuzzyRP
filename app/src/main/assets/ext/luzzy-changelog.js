...
### v1.2.1 — 记忆链路修复与内容管理器 × 品牌色收编 × 上游标记体系（🚧 待发布 · 上游基线 RP-Hub 1.8.9）

> **状态：功能完成，布局异常已修复（模拟器五页核对 + 管理器 CRUD 回归通过）；
> 正式包构建与 GitHub Release 待发，真机复验建议随正式包进行。**
...
- **上游标记体系（硬性规定 10）**：全部二创改动在上游文件内携带 \`[LuzzyRP patch NNN]\`
  ...
  当前态逐文件，007/009/012-018 全覆盖）并接入 \`apply-patches.ps1\` 自动重放判定；
...
**注意事项**
- 真机复验（小米 df97f3c4）建议随 v1.2.1 正式包进行；本版 EXTRACT_VERSION=14，
  安装即自动重新解压资产。
