...
| 输入岛：模型 chip | ✅ 本轮做 | `GET {base}/models` 真实拉取 + 写回配置（`chat/ModelCatalog.kt`） |
| 输入岛：工具 | ✅ 本轮做 | 开关真实影响请求体（`TransportConfig.toolsEnabled`） |
| 输入岛：世界书 | ✅ 本轮做（**只读**） | 现有真实条目可展示；编辑属用户数据 |
| 输入岛：附件 | ⛔ 留 P5 | 需要图片管线（SAF + base64 + `image_url` part + 存储） |
| 输入岛：预设 | ⛔ 留 P4 | 预设是**用户数据**，无存储层只能做假壳 |
| 输入岛：工作区 | ⛔ 留 P5 | 上游是一整个特性（工件/文件工作区） |

**后三项在实现前不放图标**：装饰性 icon 是 huashu 明令的 slop，而「点不动的图标」更是欺骗——
两者都比「暂时没有这个入口」更差。
...
### 18.3 输入岛重排（用户报「太臃肿」后按上游实测重做）

**症状**：5 个 48dp 图标铺满一行 + 行内模型 chip → 超出岛宽，chip 被挤成两行并**压到图标行上**
（截图存档 `verify-p3-island-before.png`）。

**上游实测**（`index.html` 1030-1077）：功能行是 4 个 `w-8 h-8`（32dp）小圆钮、图标 16dp，
且**模型选择器根本不在这一行**（在「快捷面板」里）。

**本版做法**：
- 功能行只留**有真实去向**的 2 个入口（世界书 / 工具），取消图标间人工间距 → 行宽从 240dp 降到 96dp；
- 模型 chip 退到行尾：`widthIn(max = 128.dp)` + `maxLines = 1` + 省略号（**溢出根治**）；
- **层级**：chip 正常态用中性底 + 次级文字，只有「未配置」才用告警色——实心珊瑚胶囊会盖过输入框，
  最强对比没给最重要的内容（视觉层级反了）；
- 内边距 10→8 / 内层 8→6 / 输入行 8→6 → 岛体高度 **126dp → 114dp**；
- 图标热区**保持 48dp**（Android 触控下限，不因紧凑而妥协）。
