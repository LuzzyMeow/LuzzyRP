 ...
 *
 * 之所以是 sealed 而不是抛异常：传输层要求**错误不打断流程**，
 * 由各协议传输实现转成 `LlmDelta(error = …)` 交上层决策。
 *
...
/**
 * 基于 OkHttp 的 SSE 流式读取器（OpenAI 协议主路径）。
 *
 * - **超时**：连接 30s、空闲（socket 读）120s；`callTimeout` 关闭
 *   （流式响应总时长不可预知，由取消信号兜底）；
...
class SseClient(
    ...
    fun post(url: String, headers: Map<String, String>, jsonBody: String): Flow<SseChunk> = flow {
        ...
        try {
            ...
            if (source == null) {
                emit(SseChunk.Failed(LlmError("响应没有正文（HTTP $code）", retryable = true, httpStatus = code)))
                return@flow
            }
    ...
    companion object {
        ...
        /** 空闲超时：OkHttp 的 socket read timeout 即「两次读到数据之间」的上限。 */
        const val IDLE_TIMEOUT_MS: Long = 120_000L

        private val JSON_MEDIA_TYPE = "application/json; charset=utf-8".toMediaType()

        /** 默认客户端：连接 30s / 空闲 120s / 不做自动重连（重试策略归各协议传输）。 */
...
/**
 * 密钥脱敏：API Key **禁止**写日志 / 异常消息。
 *
 * 保守策略：`sk-` / `AIza` 前缀串、`key/token/authorization/bearer` 后的取值一律打码，
 * 并截断长度，避免把整个响应体写进日志。
 */
internal fun redactSecrets(text: String, maxLength: Int = 400): String {
    var out = text
        ...
        // Google API Key 形态（Gemini 的密钥走查询串，最容易从错误体里漏出来）
        .replace(Regex("AIza[A-Za-z0-9_\\-]{10,}"), "AIza***")
        .replace(Regex("\\s+"), " ")
        .trim()
