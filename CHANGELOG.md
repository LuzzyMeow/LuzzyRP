...
### v1.2.3 — 同步上游 1.9.0 × 用量趋势图 × 关于页工具化 × 记忆检索修复 × 设置页清理（上游基线 RP-Hub 1.9.0）
...
- EXTRACT_VERSION 20→22，安装即自动重新解压资产。
- 同步门与标记门：apply-patches 重放全部幂等 SKIP + verify-markers **56 PASS / 0 FAIL**
  （新增 021-026 共 13 项校验）；6 个实体 diff 再生成（范围式命名 007-023 / 009-023 /
  012-026）并全部通过逆向 `--check` 校验；指纹基线 1.9.0（94a0cd9）。
- 向量相似度阈值仍为硬编码 0.45（app.js），可调阈值滑杆列候选迭代。
- 本版实施前按硬性规定 9 复读 4 项设计 SKILL；开屏动画改版三方向设计板产出中
  （docs/design/splash-v1/），用户选定后落地并补记。
