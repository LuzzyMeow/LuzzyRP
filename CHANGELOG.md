...
- **输入岛排版臃肿 + 模型溢出**（2026-09-12 用户反馈「输入框的组件前端排版设计太臃肿」）：
  实测根因是「5 个 48dp 图标 + 行内实心模型胶囊」超出岛宽，胶囊被挤成两行并**压到图标行上**。
  第一次修偏了——把「太挤」当成「组件太多」而**删掉了附件/预设/工作区三个入口**，
  用户当场纠正「我说的臃肿不是叫你删掉组件啊，我让你重新设计一下输入框的各部分内容」。
  定稿为**入口全部保留 + 重排分区与视觉层级**：功能行 5 个入口统一 48dp 热区 / 17dp 图标、
  彼此相邻不留缝；模型从实心珊瑚胶囊降为**纯文字 + 下拉箭头**（最强对比让给输入框与发送键，
  箭头补上可供性）；输入行仍是主体 + 唯一实心发送键；内边距收紧 → 岛体 **126dp → 114dp**。
  未实现的三项（附件/预设/工作区）**入口保留、点击给如实说明**（「需要 P5 的图片管线，届时开放」）。
