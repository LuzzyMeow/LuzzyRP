...
| **真机上禁止改动用户数据（会话 59 血证：用户会话被重置）** | 真机性能测量时用 CDP 往 `chatHistory` 里 push/pop 合成消息 + 反复置位 `isGenerating`，结果**用户的 13 条会话被重置成 1 条**（只剩角色 `first_mes`），IndexedDB 里也已落盘、**不可恢复**（memory 分片同为空）。期间有一次探针在 teardown 前崩溃，把「合成消息 + `isGenerating=true`」留在真机数分钟，最可能是诱因。**纪律**：① 真机探针一律**只读**；确需写入时**先在应用内建一次性角色/会话**，或改在桌面同引擎族 + 桩数据上测；② 收尾必须是**独立的一次调用**（先复原再断言）；③ 每步之后核对被改动的状态；④ 不要用 `indexedDB.open(不存在的库名)` 去「探测」——那会**创建**空库（本轮误建 5 个，已删） |
