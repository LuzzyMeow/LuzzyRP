...
## 会话 24 · v1.4.0 上游 1.9.2 同步（2026-09-08）

**开始**：用户指示「上游节点已更新，探索此次合并更新是否安全」→ 调查完成（四路分析：
公告/nsfw 完整性、diff 碰撞面、patch 锚点存活、结构断言）→ 用户确认「完美的合并计划」→
`docs/PLAN-v1.4.0.md` 定稿 → 用户拍板 4 决策（D1-A 全屏继续下线 / D2-A 抗截断采纳上游
output_reply 协议 / D3-A 保留「开卷」开屏 / D4-A 新功能默认值原样）→ 开始执行。

**调查结论**：上游 1.9.2（d2f2625，14 提交，9 文件 +2242/-2268）。nsfw 对象逐字节一致 ✅；
vendor 零变化 ✅；novel/runtime-services 零变化 ✅；无新增/删除文件 ✅。碰撞面：约 30 个
patch 需重放，app.js（-1398 行）与 index.html（1521 行）双高危区；上游恢复全屏（022 冲突）、
抗截断改 output_reply 工具协议（015 融合）、getImageTagRegex 签名去参（015 调用点适配）、
fetchModels 恢复无条件调用（012 懒拉取重放）、TokenUsageView 无折线图（025 重放）、
