...
- **输入岛排版臃肿 + 模型 chip 溢出**（2026-09-12 用户反馈「输入框的组件前端排版设计太臃肿」）：
  实测根因是「5 个 48dp 图标 + 行内模型 chip」超出岛宽，chip 被挤成两行并**压到图标行上**。
  按上游实测重排（上游这一行是 4 个 32dp 小圆钮，且**模型选择器不在这一行**）：
  功能行只留有真实去向的 2 个入口并取消人工间距；chip 退到行尾并限宽 128dp + 单行省略；
  内边距收紧 → 岛体 **126dp → 114dp**；热区仍保持 48dp。
  同时修正层级：chip 正常态改中性底/次级文字（实心珊瑚胶囊会盖过输入框）。
...
- **输入岛功能入口真实现**（并撤掉尚未实现的入口）：
  ...
  - **附件 / 预设 / 工作区**：依赖 P5 图片管线、P4 数据层、P5 工作区特性，**实现前不放图标**
    （点不动的图标是欺骗；装饰性 icon 也是明令的 slop）。
