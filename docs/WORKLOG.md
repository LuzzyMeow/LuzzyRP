...
5. 正文渲染是轻量分段器（无 Markdown 内联/列表/代码块），P5 接完整渲染器；
6. 会话仅内存态（杀进程即失），P4 数据层后持久化；会话列表页仍是静态稿。

### 五、下一步

- **P3 发送链路完整化**：操作行接线（复制 / 编辑 / 重新生成=累积分支）+ 停止语义化 +
  模型 chip 切换；随后 P4 数据层与 IndexedDB 迁移（PLAN §7 标注的最高风险项）。
