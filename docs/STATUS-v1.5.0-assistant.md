...
> **最近更新**：2026-09-11 13:10（会话 56 · **助手导航扁平化**：8 个子项各自独立单页、均从侧栏进）
...
## 8. 真机验收进度
    ...
    页面转场        ██████░░░░  ~60%  （桌面门禁全绿 + 真机逐帧实测 RP 页间淡化生效；
                                     会话 57 的三处助手侧修复**已装机未复验**）
...
**复验清单（下一会话按此逐条走，勿重新推导）**：

1. 装机/重启后**先点掉开屏**（`.lsp-dive-btn` ≈ 坐标 610,1284），否则页面被点击门挡住；
2. 侧栏点「助手 → 设置」→ 覆盖层淡化进入（页内不应再有第二次滑动/淡化）；
3. 在设置页点左上角汉堡**一次** → 侧栏应立刻滑入（与覆盖层淡出同时，约 200ms 完成）；
4. 侧栏点「助手 → 对话」→ **必须落到对话页**（会话 57 修复点 ①）；
5. 侧栏点「记忆系统」（RP 页间）→ 旧页淡出 + 新页淡入 + 侧栏左移同时进行；
6. 会话页点一条已有会话 → 落到对话页且会话正确（会话 56 扁平化遗留）。
...
| 23 | **「还是有很大的问题：页面之间的切换还是硬切换 / 点助手子项『聊天』落到设置页 / 进入设置页后点汉堡无法呼出菜单」** | 三处根因与修复见会话 57：① 进入跳转的 ChatList 守卫 + `LaunchedEffect` key 不变 → 改自增 `navSeq`；② 覆盖层淡化与页内动画**叠两套** → 进入期间页内不做动画；③ 摘类导致上游 `isMobileSidebarOpen` 与 DOM 脱节 → 改点上游汉堡。**顺带**：呼出侧栏改 200ms 交接令牌 + 按返回值重试 | ✅ 代码完成并装机；**真机复验待做（用户指示暂停）** | 会话 57 |
| 24 | **「期望：呼出侧边菜单栏 → 随点击切换至其他页面，并且同步全局的页面切换效果」** | 统一编排现覆盖三条路径：① RP 页间（侧栏点页）＝侧栏左移 + 内容交叉淡化；② 侧栏 → 助手＝侧栏左收 + 覆盖层淡入（页内不再叠动画）；③ 助手 → 侧栏＝覆盖层淡出（200ms 交接令牌）+ 侧栏滑入，同帧起跑同时结束。契约见 `DESIGN.md §动效「页面交接」` | ✅ 代码完成；**真机手感待复验** | 会话 57 |
| 25 | （待用户提出下一处） | — | — | — |
