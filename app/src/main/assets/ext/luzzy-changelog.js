...
- **P4-B 存储选型与数据层**（2026-09-12，会话 70）
  - **存储定为 Room**，但**结论与计划里的预判相反**：计划担心「AGP 9 内置 Kotlin → KSP 装不上」，
    实跑后 KSP \`2.3.9\` + Room \`2.8.5\` 的 \`kspDebugKotlin\` **真的执行了**、编译通过、
    装到模拟器跑仪器化测试 **9 例全绿** → 悲观前提**被证伪**。
    于是决策改按**写模式**立论：消息是唯一高频增量写的数据，整段会话存一列 JSON 会让每次落盘
    重写整段历史（重度用户数 MB）→ 一消息一行 + \`scopeId\` 索引。理由与前提写死在
    \`docs/DESIGN-migration.md\` §8，含「哪一天把消息塞回 JSON 列就该重估」的触发条件。
  - **数据层** \`data/store/\`：八张表（角色 / 分支 / 分支元信息 / 消息 / 记忆 / 记录集合 / kv / 附件）
    + \`LuzzyStore\` 门面 + \`MigrationWriter\`。要点：消息主键用 **\`(scopeId, sortIndex)\`（位置）**
    而非消息 id（\`first_mes\` 那条通常没有 id，顺序才是旧数据的真实身份）；
    \`payload\` 列按**旧结构键名逐字**背其余字段 → 迁移零丢失、日后加字段不必改表；
    导入语义是「一个事务里先清内容表再全量写」→ 幂等；**附件先落盘再进事务**
    （反过来会出现「事务回滚了但文件已在」这类坏状态）；迁移标记由上层在成功后写。
  - **测试**：\`LuzzyStoreTest\`（仪器化，只跑模拟器）9 例 —— 迁移→入库计数一致、重复导入不翻倍、
    **关库再开数据仍在**（杀进程重启的最小证据）、迁移标记不被内容清表带走、
    追加只影响自己那一行、尾部删除、附件只写一次、跨角色作用域列举、空库可读。
  - 门禁：单测 **335** + 仪器化 **16**（7 聊天 UI + 9 数据层）全绿，\`checkChat\` 一线跑通。
  - **UI 接入真实存储（3.4）**：聊天页会话数据从「内存 + 内置演示」改为真实存储 ——
    启动即读（含**分支按 id 读**）、发送/编辑/删除/分支操作落盘、**思考内容复原成思考节点**
    （否则重启后消息会「少一块」）。界面不等 IO（写走独立协程），但**错误不吞**（打日志，
    否则「没存住」会表现为「重启后少一条」，那时再查就晚了）；空库回落演示态且**不落盘**
    （没有宿主角色 → 不写半套数据）。
    新增 \`ChatPersistenceTest\`（仪器化 5 例）：启动读到存储而非演示 / 发送落盘 / 编辑只改 content
    且 **payload 一字不动**（旧结构多余字段不丢）/ **关连接同一文件重开仍在**（「杀进程重启」的
    最小证据，含分支与思考节点）/ 分支切换落盘。两个测试接缝（\`ChatPage(sessionRepository=…)\`、
    \`engineFactory\`）——不注入的话测试会去读设备上真实的 \`luzzy.db\` 并真发网络请求，那是隐藏耦合。
  - **测试写法上的四条坑**（全部实测踩到，已写入 \`docs/CHAT-REGRESSION.md\` §4）：
    ① \`Thread.sleep\` 轮询会**饿死帧**（落盘协程在帧驱动的 UiDispatcher 上，于是「界面出来了、库里没写」，
    还留下 JobCancellationException 伪装成数据库坏了）；② \`performTextInput\` 是追加不是替换
    （改已有文字要先 \`performTextClearance\`）；③ \`substring = true\` 会让「分支首句」命中「首句」的断言（假绿）；
    ④ 一个回合会追加两条消息（用户 + 助手），**别用「条数 +1」断言**，改成按内容找。
  - 另记一条**测试自省**：持久化用例最初把「启动在主线」写死，结果全红 —— 实际样例数据的
    \`activeBranchId\` 是 b1，界面**正确地**打开了分支。**是测试的假设错了，不是应用错了**；
    修好断言后它反而变成了「分支按 id 读」的验收点。
- **P4-C 迁移入口接线完成 —— P4 全链路端到端跑通**（2026-09-12，会话 70）
  - \`MigrationRunner\`：隐藏 WebView **复用 \`WebViewSetup.configure\`** 打开 \`ext/luzzy-migrate.html\`
    → 导出页读两库、分块经桥回传 → 收件箱拼接 → \`LegacyDb.parse\` → \`LegacyMigrator.migrate\`
    → \`MigrationWriter.import\` → **成功后才写完成标记**（中断可续的前提）。
  - \`MigrationCoordinator\`：进程内只跑一次的浅状态机。界面**等它出结论再读库** ——
    不等的话会出现「迁移还在写、聊天页已按空库渲染成演示角色」，用户先看到演示数据、
    真数据随后悄悄出现。**迁移失败不阻断启动**（进空库/演示态，下次启动重来）。
  - **设备端端到端实测**（模拟器，真实旧数据）：\`导出页回调 ok=true chunks=1 chars=82992\`
    → \`迁移完成：已从旧版迁移：角色 5 · 会话 16 条 · 记忆 8 · 世界书 4 · 预设 19\`，
    与 JVM 夹具测试的期望**完全一致**（两条独立路径同解）；重启后**不再迁移**、数据仍在；
    界面顶栏与名牌显示**迁移进来的角色**（\`last_active_char=1\` 的「下标 → uuid」换算生效）。
  - 迁移报告只用**既有 Snackbar 提示一行**（\`角色 N · 会话 M 条 · …\`）；专门的报告页是视觉产出，
    按硬性规定 9 需走设计流程，**留待设计**（明细已存 kv \`legacy.migrationReport\` 与日志）。
  - **顺带补齐顶栏角色名/状态行**：DESIGN-compose 的顶栏条款本就要求显示「角色名 + 状态行 + 角色图」，
    此前一直填内置演示角色（内容是一个角色、标题是另一个角色）。
  - **迁移顺带暴露的两个真实缺口**（迁移的价值正在于此，均归 P5）：① 正文里的 \`{{char}}\`/\`{{user}}\`
    占位符未被替换（上游在渲染期做，Compose 侧还没实现——**数据是对的，渲染少了一步**）；
    ② 顶栏头像与背景仍是内置演示图（存储里有真实头像，需接入位图解码）
- **P4-B-3.3 设置持久化 + P4-C 验收标准**（2026-09-13，会话 71）
  - **主题模式持久化**：此前亮/暗只是进程内状态（\`mutableStateOf\`），重启即回系统默认；
    现在落 \`SharedPreferences\`（\`luzzy_settings\`），「跟随系统」是默认，手动切过就记成显式值。
    **为什么设置不放 Room**：主题必须在**首帧之前**就知道（Compose 第一次组合就要定色板），
    而 Room 是协程式 API，拿到值前界面已经画出去 → 一次可见的「先亮后暗」。分工写进类注释：
    **用户内容 → Room；应用设置（同步读）→ SharedPreferences**。
  - **旧设置一次性搬运**：\`LegacySettingsReader\`（**纯函数**，8 例单测直接用真夹具的
    \`rp_hub_settings\`）+ \`SettingsBootstrap\`。要点：旧结构把「供应商 URL」散在
    \`apiProviderOverrides\` / \`apiProviders\` / \`apiUrl\` 缓存**三处**，优先级不能乱（缓存只在
    「它就是当前供应商」时才可信，否则会把 A 商的地址配到 B 商头上）；密钥优先取分商槽位。
    搬运是**一次性**（\`legacy_imported\` 标记）且**只补空缺**——否则用户在新版改过的选择
    会在下次启动被旧值打回去，那比不迁移更烦人。
  - **会话总览的数据层**：\`ChatSessionRepository.overview()\` 给出每角色 × 每分支的摘要
    （条数 + 末条预览，主线优先、**空分支也在**）。末条预览走
    \`ORDER BY sortIndex DESC LIMIT 1\` 单条查询而不是「取全部再取末项」——总览要为每个分支各取一次，
    搬全量历史会让「打开一个列表」变成搬几 MB。呈现层未做（新页面属视觉产出，见下）。
  - **验收标准写死** \`DESIGN-compose §20\`：13 项已收口（逐项带证据） + 7 项**明确标为未验收**
    （真机覆盖安装 / 总览页 / 预设与世界书编辑 / 用户可调字号 / 迁移报告页 / 占位符替换 / 真实角色图），
    每项都写明**为什么还没做**，免得日后被当成已完成。
  - 覆盖安装（模拟器）实证：多次同签名 \`install -r\` 之后旧 WebView 库仍完好
    （\`RPHubDB\` 29 键 + \`SillyTavernDB\` 3 键、三张角色卡读回）——迁移只读旧库，数据从未被改动。
  - 门禁：单测 **343** + 仪器化 **26** 全绿。
  - **坑（记进文档）**：**仪器化测试方法名不能带空格**——反引号中文用例名在 JVM 单测里没事，
    但 androidTest 要过 D8，Kotlin 为其中 lambda 生成的内部类名含空格，
    DEX 040 之前不允许 → \`dexBuilderDebugAndroidTest\` 失败，且报的是 R8 内部类名，
    完全看不出与测试名有关。androidTest 一律 ASCII camelCase。

- **P4-C 会话总览页（三方向设计门 → 实现）+ 性能专项**（2026-09-13，会话 71）
  - **设计门**：按硬性规定 9 出三方向**真实渲染**（\`docs/design/boards-v5/\`：SPEC + 3 HTML + 亮暗 6 图 + 速览 +
    \`direction-approved-v5.md\`），用户选定 **B · 分组行**（现实参照 Things 3）。
    A 方向（秒数轮盘抽中的孟菲斯拼贴）在语境内诠释时**主动声明降级**（做旧肌理纯 CSS 无法还原，不假装做出）。
  - **实现**：角色=粘性组头、分支=等高行（≥68dp）；预览取**最后一条用户发言**（用户拍板）；
    空会话可见（斜体「还没开始」）；当前会话 primary 圆点；入口在**聊天页顶栏**（\`Sessions\` 路由 \`inDrawer=false\`，
    侧栏不加项）；点一行写 kv 再回聊天页 ⇒「点一行 = 回到那一段」跨页不传状态。
  - **数据层性能**：\`overview()\` 从「每会话查 3 次」（90 条会话实测 **332ms**）改为**恒定 3 次查询** →
    **9ms**；\`load()\` 从「装载该角色所有分支全部消息」改为**只装当前分支**（重卡 6000 条 51→37ms），
    其余分支切换时按需装入。专用剖面 \`PerfProfileTest\`（30 张卡/90 会话/**1.24 万条消息**）钉住基线。
  - **修两个真实缺陷**（都是「迁移进来的真数据」暴露的）：
    ① **思维链被当正文渲染** —— 上游把 CoT 存在正文里（\`<thinking>…\`，渲染期靠 \`parseCot()\` 剥离），
    我们当初只认独立 \`reasoning\` 字段 → 新增 \`CotParser\`（上游语义的 Kotlin 移植：成对栈+嵌套、
    围栏/行内码/HTML 块/注释里的标签不算、流式半截标签不显示），载入时把内联 CoT 变成思考节点、
    编辑时 \`rewrap\` 把思维链按原样装回（不让「改几个字」抹掉那次生成）；
    ② **每帧重解析** —— \`body\` 一度是 getter，每次重组都重新正则扫 1~2KB 正文；
    改为构造时算一次 + 有界缓存（冷 **193µs** / 热 **0.44µs**）。
  - **渲染期替换 \`{{char}}\`/\`{{user}}\`**（上游只替换后者）：存储不动、\`raw\` 逐字保持一致。
  - **诊断出「模拟器卡」的真因（不是应用）**：拿**系统设置页当基线**——同输入下它也是 200ms 中位帧 / 72% 掉帧，
    而 AVD 只配了 **1536M RAM**；提到 **4096M** 后：系统设置 17ms/2.2%、本应用聊天页 17ms/**2.6%**，
    **整套仪器化测试耗时 2m40s~3m28s → 54s**，之前几次「偶发红」随之消失。
