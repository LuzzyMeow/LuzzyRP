...
/**
 * SSE 分帧（纯 Kotlin、无 Android / OkHttp 依赖，可单测）。
 *
 * 协议要点（W3C EventSource 子集，OpenAI / Anthropic / Gemini 流式响应使用的部分）：
 * - 帧（event）之间以**空行**分隔：`\n\n` / `\r\n\r\n` / `\r\r`；
 * - 载荷由 `data:` 行给出，同一帧内多行 `data:` 以 `\n` 连接；
 * - 以 `:` 开头的行为注释（keep-alive），`event:` / `id:` / `retry:` 一律忽略；
 * - 帧可能跨 chunk 边界，[Parser] 负责缓存半帧。
 *
 * 本对象只做**分帧**，不做 JSON 解析（JSON 归 [OpenAiWire] / [AnthropicWire] / [GeminiWire]）；
 * `[DONE]` 哨兵原样返回，由调用方判定流结束。
 *
