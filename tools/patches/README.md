...
#
# 036-memory-manager-live.patch（2026-09-06，v1.3.0 用户真机反馈）
#   - app.js: 记忆内容管理器实时联动 watch——面板展开且作用域为当前会话时，
#     memories/classicMemories 长度变化（补录新增分片/总结）即时重快照进管理器列表
#     （017 原实现为打开时一次性快照，补录完成后不刷新——真机实测反馈）
#   - 对应：用户真机反馈（配置嵌入模型后补录成功但管理器不实时更新）
#   - 预期冲突点：上游改记忆管理器结构时需重打
#
