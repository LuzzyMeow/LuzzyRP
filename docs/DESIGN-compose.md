...
### 18.3 输入岛重排（用户报「太臃肿」→ 两轮才对）

**症状**：5 个 48dp 图标铺满一行 + 行内实心模型胶囊 → 超出岛宽，胶囊被挤成两行并**压到图标行上**
（截图 `verify-p3-island-before.png`）。

**上游实测**（`index.html` 1030-1077）：功能行是 4 个 `w-8 h-8`（32dp）小圆钮、图标 16dp，
且**模型选择器不在这一行**（在「快捷面板」里）。

**❌ 第一轮（错误）：把入口删掉**。我删掉附件/预设/工作区三个入口、只留世界书与工具。
用户指正：「我说的臃肿不是叫你删掉组件啊，我让你重新设计一下输入框的各部分内容」。
——**把「太挤」误判成「东西太多」，用删功能解决排版问题，是改需求。**

**✅ 第二轮（定稿）：保留全部入口，重排分区与视觉层级**：

```
功能行：[附件][预设][世界书][工具][工作区] ← 次级工具，统一规格、彼此相邻（热区不重叠）
                                     deepseek-flash ⌄ ← 状态信息，最弱一级（纯文字+箭头，无底色）
输入行：[ 写点什么……              ]  [ ➤ ] ← 主体与主操作（唯一实心色块）
```

- **功能行**：5 个入口统一 48dp 热区 / **17dp 图标**、彼此相邻不留缝（热区不重叠、视觉成簇）；
- **模型**：从实心珊瑚胶囊降为**纯文字 + 下拉箭头**——「最强对比」让给输入框与发送键
  （此前胶囊比输入框还抢眼，层级是反的），箭头同时补上「可点开选择」的可供性（此前无任何提示）；
- **输入行**：正文 14sp 是主体，发送键是唯一实心色块；
- 内边距 10→8 / 内层 8→6 / 输入行 8→6 → 岛体 **126dp → 114dp**；图标热区**保持 48dp**
  （Android 触控下限，不因紧凑而妥协）。

**已知的剩余取舍（留给用户定）**：视觉上图标簇仍比上游松，因为上游用 32dp 热区而我们要守
Android 48dp 下限（图标 17dp → 相邻图标边缘间距 31dp）。若接受低于 48dp，可把热区收到 40dp
（簇宽 240→200dp、边缘间距 23dp、更接近上游观感）。**默认不动**：合规优先，等用户明确取舍。
