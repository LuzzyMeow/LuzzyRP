 ...
 * 替代 `mockwebserver`（本机 Gradle 缓存有制品，但 `app/build.gradle.kts` 未声明该依赖，
 * 任务约束禁止改构建脚本）。
 *
...
class RawSseServer(private val scripts: List<Script>) : Closeable {
     ...
     * @param extraHeaders 追加响应头（每行含 CRLF），如人为声明过大的 `Content-Length`。
     */
    data class Script(
        val status: Int = 200,
        val body: String = "",
    ...
    val url: String get() = "http://127.0.0.1:${serverSocket.localPort}/v1/chat/completions"
