...
### v1.2.1 — 记忆链路修复与内容管理器 × 品牌色收编 × 上游标记体系（🚧 开发中 · 未发布 · 上游基线 RP-Hub 1.8.9）

> **状态：开发中，不可正常游玩。** 当前构建存在**布局异常**（部分页面顶部内容缺失、
> 底部溢出屏幕边缘，详见「注意事项」），由 v1.2.1 结构改动引入，**待下一轮修复**；
> 修复进度与诊断线索见 `docs/WORKLOG.md` 会话 14 移交补充节。
...
- **上游标记体系（硬性规定 10）**：全部二创改动在上游文件内携带 `[LuzzyRP patch NNN]`
  ...
  当前态逐文件，007/009/012-017 全覆盖）并接入 `apply-patches.ps1` 自动重放判定；
...
**注意事项**
- 🚧 **已知未解问题（移交优先项）**：EXTRACT 13 构建存在布局异常——部分页面
  **顶部遗漏字段、底部超出屏幕边缘**；怀疑为 v1.2.1 对 index.html 的结构改动
  在 memory 视图之后区域缺失闭合标签所致（正则计数曾出现 -2 信号被误放行）。
  诊断线索与修复原则见 `docs/WORKLOG.md` 会话 14 移交补充节。
- 已修复待复验：**记忆面板悬浮于所有页面底部**（v1.2.1 结构改动曾使记忆视图的
  引擎设置/向量检索卡脱离视图 v-if 常驻渲染；已通过基线还原+重放修复，
  模拟器三页验证通过，真机复验因布局异常未完成）。
